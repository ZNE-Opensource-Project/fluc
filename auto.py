...
from default import check, cross
...
log = logging.getLogger('fluc.auto')
...
async def on_member_join(member: discord.Member):
    ...
    if isinstance(channel, discord.TextChannel):
        await channel.send(f'Welcome to Fluc {member.mention}. Invite nake bot: <#{config['invite_channel']}>')

@bot.command('stats', aliases=['ping'])
@command(keep_message=True)
@cooldowns.check(1)
async def cmd_stats(ctx: commands.Context, user: User, *args):
    embed = discord.Embed(title=f'{__name__}')
    embed.add_field(
        name='Connection',
        value=(
    ...
    disk = disk_usage('./')
    used = round(float(utils.convert_bytes(disk.used)[0]), 2)
    total = round(float(utils.convert_bytes(disk.total)[0]), 2)
    embed.add_field(
        name='Stats',
        value=(
            f'Memory Usage: 1GB/24GB\n'
            f'Disk Usage: {used}/{total}'
        )
    )
